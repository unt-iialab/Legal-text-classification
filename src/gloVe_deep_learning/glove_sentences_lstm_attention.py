...
f_glove = open(r"data/glove_sentence.csv",'r')
...
for i, (train_index, val_index) in enumerate(kf.split(data_1, labels_1.argmax(1))):
    ...
    print("each corss-validation results")
    print("accuracy_score: %.2f%%" % (a_score * 100))
    print("Precision: %.2f%%" % (p_score * 100))
    print("Recall: %.2f%%" % (r_score  * 100))
    print("f1_score: %.2f%%" % (f_score * 100))
